...
import {
    Lead, ActivityLog, LeadStatus, OutreachFlowTab,
    FollowUpSequence, ChatMessage, DeadReason, OutreachStep
} from "../../types";
...
interface LeadsStore {
    ...
    saveFollowUpSequence: (id: string, sequence: FollowUpSequence) => Promise<void>;
...
export const useLeadsStore = create<LeadsStore>()(
            ...
            fetchLeads: async () => {
                ...
                if (!error && data) {
                    const leads: Lead[] = data.map(r => ({
                        ...
                        lastOutreachStep: r.last_outreach_step,
    ...
    if (lead) {
        await get().logActivity({
            ...
            setLastOutreachStep: async (id, step) => {
                set(state => ({
                    leads: state.leads.map(l =>
                        l.id === id ? { ...l, lastOutreachStep: step } : l
                    ),
                }));

                await supabase
                    .from("leads")
                    .update({ last_outreach_step: step })
                    .eq("id", id);
            },
