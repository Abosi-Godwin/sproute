...
import {
    Search, BookmarkCheck, MessageCircle,
        ...
        <div className="flex flex-col items-center text-center gap-6">
            ...
            <div className="space-y-3">
                ...
                <p className="text-base-400 text-base max-w-sm leading-relaxed">
                    Find local businesses, reach out on WhatsApp and land clients — all in one place.
                </p>
            </div>
            <button
                onClick={onNext}
    ...
    const steps = [
        {
            ...
            description: 'Find businesses by category — restaurants, salons, pharmacies and more.',
        },
        {
            icon: BookmarkCheck,
            title: 'Save hot leads',
            description: 'Sproute scores each business and shows you who is worth contacting first.',
        },
        {
            icon: MessageCircle,
            title: 'Generate and send',
            description: 'Get a personalised WhatsApp message written for each business in one tap.',
        },
    ];
...
function Step3({ onNext }: { onNext: () => void }) {
    ...
    const quickPicks = [
        'Web development', 'Photography', 'Plumbing',
        'Catering', 'Graphic design', 'Electrical',
        'Video editing', 'Social media management',
    ];

    const handleNext = () => {
        if (value.trim()) setServiceDescription(value.trim());
        onNext();
    };
        ...
        <div className="flex flex-col items-center text-center gap-6 w-full max-w-sm">
            ...
            <div className="space-y-2">
                ...
                <p className="text-base-400 text-sm max-w-xs leading-relaxed">
                    Sproute uses this to write personalised outreach messages for each business you contact.
                </p>
            </div>
            ...
            <div className="w-full space-y-3">
                <input
                    type="text"
                    placeholder="e.g. Web development, Photography, Plumbing..."
                    value={value}
            ...
            <div className="flex flex-col items-center gap-2 w-full">
                ...
                <button onClick={onNext} className="text-xs text-base-500 hover:text-base-300 transition-colors">
                    Skip for now
                </button>
            </div>
        </div>
...
function Step4({ onNext }: { onNext: () => void }) {
    const { portfolioUrl, setPortfolioUrl } = useSettingsStore();
    const [value, setValue] = useState(portfolioUrl);

    const handleNext = () => {
        if (value.trim()) setPortfolioUrl(value.trim());
        onNext();
    };
    ...
    return (
        <div className="flex flex-col items-center text-center gap-6 w-full max-w-sm">
            ...
            <div className="space-y-2">
                ...
                <p className="text-base-400 text-sm max-w-xs leading-relaxed">
                    When prospects ask to see examples, Sproute includes your portfolio link automatically.
                </p>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 w-full">
                <button
                    onClick={handleNext}
                    className="w-full flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-medium px-6 py-3 rounded-xl transition-colors"
                >
                    Continue
                    <ArrowRight className="w-4 h-4" />
                </button>
...
export default function OnboardingOverlay() {
    ...
    const steps = [
        ...
        <Step4 onNext={() => setStep(4)} />,
        <Step5 onFinish={handleFinish} />,
    ];
